...
async function newGitLabNote (req, res, next) {
  ...
  models.Note.parseNoteId(req.alias, async function (err, _id) {
    if (!err && _id) {
      res.redirect(config.serverURL + '/' + req.alias)
    } else {
      if (!req.body) {
...
async function retrieveGitLabFile(projectId, branch, filePath, accessToken) {
  return executeGitLabApiRequest('/projects/' + encodeURIComponent(projectId) + '/repository/files/' + encodeURIComponent(filePath) + '?ref=' + branch + (accessToken ? ('&access_token=' + accessToken) : ''))
}
