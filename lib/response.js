...
async function retrieveGitLabFileContentByGitLabUrl(req, res, gitlaburl) {
  ...
  if (req.isAuthenticated()) {
    ...
    const projectInfo = await retrieveGitLabProjectInfo(projectPath, user.accessToken)
    const file = await retrieveGitLabFile(projectInfo.id, branch, filePath, user.accessToken)
...
async function retrieveGitLabProjectInfo(projectPath, accessToken) {
  return executeGitLabApiRequest('/projects/' + projectPath.replace(/\//g, '%2F') + '?access_token=' + accessToken)
}
async function retrieveGitLabFile(projectId, branch, filePath, accessToken) {
  return executeGitLabApiRequest('/projects/' + projectId + '/repository/files/' + filePath + '?ref=' + branch + '&access_token=' + accessToken)
}
