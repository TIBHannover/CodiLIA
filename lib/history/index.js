...
var worker = require('../workers/liaWorker')
...
function defines (code) {
  var app = worker.init({ flags: { cmd: code } })
  ...
  return new Promise(function (resolve, reject) {
    app.ports.output.subscribe(function (data) {
      ...
      if (ok) {
        ...
        reject(new Error('head could not be parsed'))
      }
    })
  })
}
...
function updateHistory (userid, noteId, document, time) {
  if (userid && noteId && typeof document !== 'undefined') {
    getHistory(userid, async function (err, history) {
      if (err || !history) return
      if (!history[noteId]) {
        history[noteId] = {}
      }

      const json = await defines(document)
      var tags = null

      try {
        tags = json.macro.tags
          .split(',')
          .map(e => e.trim())
      } catch (e) {
        tags = []
      ...
      noteHistory.tags = tags
      noteHistory.liascript = json
